...
const loadArticles = async (page = 0) => {
  ...
  try {
    const response = await fetch(`http://luren.online:2345/proxy/page?pageNum=${page}&pageSize=${pageSize}`)
...
const getImageUrl = (filename) => {
  ...
  return `http://luren.online:2345/proxy/image?filename=${filename}`;
}
...
const loadMarkdownContent = async (filename) => {
  ...
  try {
    const url = `http://luren.online:2345/proxy/md?filename=${encodeURIComponent(filename)}`
...
const likeArticle = async (timestamp) => {
  try {
    ...
    const response = await fetch('http://luren.online:2345/proxy/likeArticle', {
      method: 'POST',
      headers: {
...
const addArticle = async () => {
  try {
    ...
    const token = localStorage.getItem('userToken')
    const response = await fetch('http://luren.online:2345/proxy/upload', {
      method: 'POST',
      headers: {
        'Authorization': token
      },
...
const deleteArticle = async (timestamp) => {
  try {
    ...
    const token = localStorage.getItem('userToken')
    const response = await fetch(`http://luren.online:2345/proxy/delete?timestamp=${timestamp}`, {
      method: 'GET',
      headers: {
        'Authorization': token
      }
...
const loadImages = async () => {
  try {
    const token = localStorage.getItem('userToken')
    const response = await fetch('http://luren.online:2345/proxy/getMdImageList', {
      headers: {
        'Authorization': token
      }
    })
...
const deleteImage = async (filename) => {
  ...
  try {
    ...
    const response = await fetch('http://luren.online:2345/proxy/deleteMdImage', {
      method: 'POST',
      headers: {
        'Authorization': token
      },
...
const viewImage = (filename) => {
  ...
  currentImageUrl.value = `http://luren.online:2345/proxy/image?filename=${filename}`
...
const uploadImages = async () => {
  ...
  try {
    ...
    const token = localStorage.getItem('userToken')
    const response = await fetch('http://luren.online:2345/proxy/mdImage', {
      method: 'POST',
      headers: {
...
const cleanInvalidFiles = async () => {
  ...
  try {
    ...
    // 1. 获取所有服务列表
    const token = localStorage.getItem('userToken')
    const servicesResponse = await fetch('http://luren.online:2345/cline', {
      method: 'GET',
      headers: {
        'authorization': token
      }
    ...
    for (const service of cleanServices) {
      try {
        const cleanUrl = `http://${service.address}`
